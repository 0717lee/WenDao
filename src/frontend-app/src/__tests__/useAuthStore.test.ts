...
describe('useAuthStore', () => {
  ...
  it('clears stored auth when /auth/me returns unauthorized', async () => {
    localStorage.setItem('texttwin_token', 'bad-token')
    localStorage.setItem('texttwin_username', 'tester')
    ...
    expect(localStorage.getItem('texttwin_token')).toBeNull()
    expect(localStorage.getItem('texttwin_username')).toBeNull()
  })

  it('keeps local auth on transient network failure', async () => {
    localStorage.setItem('texttwin_token', 'good-token')
    localStorage.setItem('texttwin_username', 'tester')
